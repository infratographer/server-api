...
func (pq *ProviderQuery) QueryServers() *ServerQuery {
	query := (&ServerClient{config: pq.config}).Query()
...
func (pq *ProviderQuery) WithServers(opts ...func(*ServerQuery)) *ProviderQuery {
	query := (&ServerClient{config: pq.config}).Query()
...
func (pq *ProviderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Provider, error) {
	...
	if query := pq.withServers; query != nil {
		if err := pq.loadServers(ctx, query, nodes,
			func(n *Provider) { n.Edges.Servers = []*Server{} },
			func(n *Provider, e *Server) { n.Edges.Servers = append(n.Edges.Servers, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedServers {
		if err := pq.loadServers(ctx, query, nodes,
			func(n *Provider) { n.appendNamedServers(name) },
			func(n *Provider, e *Server) { n.appendNamedServers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProviderQuery) loadServers(ctx context.Context, query *ServerQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *Server)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(server.FieldProviderID)
	}
	query.Where(predicate.Server(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(provider.ServersColumn), fks...))
	}))
...
func (pq *ProviderQuery) WithNamedServers(name string, opts ...func(*ServerQuery)) *ProviderQuery {
	query := (&ServerClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedServers == nil {
		pq.withNamedServers = make(map[string]*ServerQuery)
	}
	pq.withNamedServers[name] = query
	return pq
}
