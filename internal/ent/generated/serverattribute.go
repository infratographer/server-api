...
// Representation of a server attribute. Server attributes describe various characteristics of a server
...
func (*ServerAttribute) scanValues(columns []string) ([]any, error) {
...
// to the ServerAttribute fields.
func (sa *ServerAttribute) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case serverattribute.FieldID:
...
// Note that you need to call ServerAttribute.Unwrap() before calling this method if this ServerAttribute
// was returned from a transaction, and the transaction was committed or rolled back.
func (sa *ServerAttribute) Update() *ServerAttributeUpdateOne {
	return NewServerAttributeClient(sa.config).UpdateOne(sa)
}

// Unwrap unwraps the ServerAttribute entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (sa *ServerAttribute) Unwrap() *ServerAttribute {
	_tx, ok := sa.config.driver.(*txDriver)
	if !ok {
		panic("generated: ServerAttribute is not a transactional entity")
	}
...
func (sa *ServerAttribute) String() string {
	var builder strings.Builder
	builder.WriteString("ServerAttribute(")
...
func (sa ServerAttribute) IsEntity() {}

// ServerAttributes is a parsable slice of ServerAttribute.
type ServerAttributes []*ServerAttribute
