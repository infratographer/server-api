...
// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (sa *ServerAttributeQuery) CollectFields(ctx context.Context, satisfies ...string) (*ServerAttributeQuery, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return sa, nil
	}
...
func (sa *ServerAttributeQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(serverattribute.Columns))
		selectedFields = []string{serverattribute.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "server":
		...
		case "createdAt":
			if _, ok := fieldSeen[serverattribute.FieldCreatedAt]; !ok {
				...
				fieldSeen[serverattribute.FieldCreatedAt] = struct{}{}
			}
		case "updatedAt":
			if _, ok := fieldSeen[serverattribute.FieldUpdatedAt]; !ok {
				selectedFields = append(selectedFields, serverattribute.FieldUpdatedAt)
				fieldSeen[serverattribute.FieldUpdatedAt] = struct{}{}
			}
		case "name":
...
type serverattributePaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []ServerAttributePaginateOption
}

func newServerAttributePaginateArgs(rv map[string]any) *serverattributePaginateArgs {
	args := &serverattributePaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &ServerAttributeOrder{Field: &ServerAttributeOrderField{}, Direction: entgql.OrderDirectionAsc}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithServerAttributeOrder(order))
			}
		case *ServerAttributeOrder:
			if v != nil {
				args.opts = append(args.opts, WithServerAttributeOrder(v))
			}
		}
	}
	if v, ok := rv[whereField].(*ServerAttributeWhereInput); ok {
		args.opts = append(args.opts, WithServerAttributeFilter(v.Filter))
	}
	return args
}
