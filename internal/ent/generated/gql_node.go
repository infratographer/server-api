...
func (c *Client) noder(ctx context.Context, table string, id gidx.PrefixedID) (Noder, error) {
	...
	case server.Table:
		...
		return n, nil
	case servercpu.Table:
		var uid gidx.PrefixedID
		if err := uid.UnmarshalGQL(id); err != nil {
			return nil, err
		}
		query := c.ServerCPU.Query().
			Where(servercpu.ID(uid))
		query, err := query.CollectFields(ctx, "ServerCPU")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
	...
	case serverchassistype.Table:
		...
		return n, nil
	case servercomponent.Table:
		var uid gidx.PrefixedID
		if err := uid.UnmarshalGQL(id); err != nil {
			return nil, err
		}
		query := c.ServerComponent.Query().
			Where(servercomponent.ID(uid))
		query, err := query.CollectFields(ctx, "ServerComponent")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		...
		return n, nil
	case servercomponenttype.Table:
		var uid gidx.PrefixedID
		if err := uid.UnmarshalGQL(id); err != nil {
			return nil, err
		}
	...
	case servertype.Table:
		...
		query := c.ServerType.Query().
			Where(servertype.ID(uid))
		query, err := query.CollectFields(ctx, "ServerType")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []gidx.PrefixedID) ([]Noder, error) {
	...
	case servercomponent.Table:
		query := c.ServerComponent.Query().
			Where(servercomponent.IDIn(ids...))
		query, err := query.CollectFields(ctx, "ServerComponent")
		if err != nil {
			return nil, err
		}
